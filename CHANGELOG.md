...
- **Browser fallback for challenge-graded downloads (F95-675j)** — when the Go path hits a Cloudflare challenge it cannot pass, moxie hands the download to the user's real browser, which performs the download itself on a copy of the live profile (same IP + UA + TLS fingerprint the clearance was minted to):
  ...
  - **Auto-detect default** — unset env installs when any supported browser exists; `MOXIE_BROWSER=auto|chrome|firefox` (per run) or `moxie config set browser_fallback auto` (persistent) forces an engine, `off`/`0`/`false` disables. **No browser is ever shipped or downloaded** — both engines launch the user's installed browser (`launcher.New()`, never rod's `NewBrowser()` download path).
  - Live-verified 2026-08-09: real Firefox 153.0.1 headless through the full pipeline (1 MiB payload, 1.65 s); installer wiring live-checked; real Chromium (Playwright cache) through the click flows (masked Continue → host button; reCAPTCHA checkbox in iframe); the real Meltys Quest download completed via the Go path (F95Zone's masked captcha is intermittent). Research round 2026-08-09 (docs/research-f95zone-recaptcha.md): the masked-URL captcha is **fully reverse-engineered** (sitekey in masked.js, token POSTs as `captcha` field — verified live), the wall is rate-based (tls-client A/B: no transport advantage), and the rod engine now injects **go-rod/stealth JS** (bot.sannysoft.com 31/31; the AutomationControlled launch flag was rejected — it core-dumps Playwright Chromium on profiles with conflicting blink prefs, e.g. Brave). **Self-hosted masked-URL defenses:** unwrap pacing (3s) + backoff retries {2s,5s,15s,45s} + `ResolveMaskedURL` browser-as-solver (click Continue → reCAPTCHA checkbox → destination URL; Go downloads with progress) — live-verified twice against the real masked pixeldrain link (~4s). **Profile-copy fix:** `Sync Data` excluded — a Brave sync LevelDB deterministically core-dumps vanilla Chromium 2-4s after launch (had been killing masked sessions all day). Known limitation: raw-launch Firefox still cannot click (no DOM access) — click-required flows need the Chrome-family rod engine (F95-1ng2); on Firefox-only machines those hosts time out.
...
- **Buzzheavier token-based resolver (F95-hs4y)** — replaces the old HTMX-only flow that got self-referential `hx-redirect`s (no token → no file link). Live-verified end-to-end 2026-08-09 (`bzzhr.to/e2yt4zd66jq3` → fafda.to → 206 file hop):
  ...
  - HTMX `GET /download?t=<token>&alt=true` with `HX-Request` + `priority: u=1, i` + Referer → `hx-redirect`; prefers the challenge-free **fafda.to** origin, rewrites dead `ts.`/`dd.` subdomain redirects to fafda.to (same `v=` token)
  - `Range: bytes=0-0` probe validating the file URL: `503 {"error":"service unavailable"}` origin outages retried with escalating backoff (1s→2s→4s→8s→8s — live-observed recovery window), `403/404` JSON errors → share-page refetch for a fresh token (bounded at 2)
  - Dead `dd.buzzheavier.com` direct pattern removed; 10 new tests (token-extraction table, retry-on-403, persistent challenge, alt=true preference + fafda rewrite, token-refetch exhaustion, 503 retry/persistent, missing-token page) + `TestBuzzheavierResolveLive` (MOXIE_LIVE-gated E2E acceptance)

- **Browser cookie reuse for download hosts (cf_clearance)** — `browser.GetCookiesForHost(hostname)` extracts cookies for any download host (RFC 6265 domain match, 60 s kooky snapshot cache, 6 KB header cap, newest-per-name dedup). The `HostResolver` merges them into every resolver GET/POST (buzzheavier HTMX, datanodes/vikingfile form flows) and the final download request, so Cloudflare-protected hosts pass their challenge when the user has visited the host in a browser. Cookies are host-scoped to the request's own domain — never sent cross-host. Best-effort: no cookies → identical behavior to before (F95-dl1)
- **Masked URL JSON unwrap** — F95Zone's masked pages are now a JS + reCAPTCHA interstitial ("Continue to \<host\>") that serves no redirect to plain HTTP clients. `unwrapMasked()` POSTs `{xhr:1, download:1}` to the masked path with the session cookie (`X-Requested-With: XMLHttpRequest`) and parses the JSON reply — `{"status":"ok","msg":"https://pixeldrain.com/u/…"}` — so masked links resolve without a browser (verified live: masked → `api/file/<id>` → 200 with the real archive). `captcha`/`error` statuses produce actionable errors; non-JSON responses fall back to the legacy GET redirect chain (F95-dl2)
- **Host identification fixes** — `bzzhr.to` (buzzheavier's short-link domain — 91 links in the library were routing as "unknown" and never hitting the HTMX resolver), `miixdrop.net`, and `proton.me` now map to the correct host labels; the buzzheavier resolver accepts 204 No Content from the HTMX endpoint per its documented contract (F95-dl3)
...
- **F95Zone search fixed — XenForo CSRF token** — XenForo 2.3+ rejects the legacy `xf_csrf` cookie value as a POST token (HTTP 400 "Security error occurred"), which silently routed every search to the Google fallback (zero results → "No results found" in the desktop browser). `SearchF95Zone` now fetches a fresh `_xfToken` from page HTML (`GET /` + hidden-input regex), caches it 10 minutes, and on a 400 invalidates and refetches once before falling back. HTTP 4xx responses are no longer retried (client errors; 5xx/429/network still retried with backoff); `NewUnsafeClient` also skips search pacing for tests. 3 new end-to-end tests with httptest servers (token fetch + cache, 400-refetch-retry, Google fallback)
- **Search result thumbnails are real cover art** — XenForo search result rows only carry the poster's avatar, which was being stretched into game-card thumbnails (blurry, wrong subject). `parseSearchResults` no longer surfaces avatars; the desktop browser attaches actual covers from the F95Checker catalog (`PublicAPI.SearchCovers`, matched by new `scraper.ThreadIDFromURL`), and threads the catalog doesn't know (mods, requests) show the placeholder instead of a stranger's profile picture
- **Full-resolution covers everywhere** — `SearchCovers` rewrites `preview.f95zone.to` (F95Checker's 400px preview CDN) to `attachments.f95zone.to` — same path, 560px+ originals (verified: 400×300 → 560×420). `extractCoverImage` now understands XenForo lazy-loaded posts: the `<img src>` is an SVG placeholder, so the real URL is read from `data-src` / the `.lbContainer-zoomer` lightbox div, and gallery thumbnails (`thumb/` subpath) are replaced by the full-size original their anchor links to. This also fixes the thread-scrape fallback and thread-preview pane, which previously returned no cover at all on modern threads
- **Browser search is explicit-only** — typing no longer auto-triggers a search (300 ms debounce removed) and remounting the tab no longer re-runs a persisted query; searches fire only from the Search button or Enter, with stale-response guards still making rapid re-searches race-safe
- **Library cover cells are landscape** — 40×56 portrait cells cropped banner covers badly; cells are now 72×40 (16:9, cover column widened 50→80px) and the detail-view cover max-width is 260→320px with a 16:9 placeholder
