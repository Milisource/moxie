...
- **Browser fallback for challenge-graded downloads (F95-675j)** — when the Go path hits a Cloudflare challenge it cannot pass, moxie hands the download to the user's real browser, which performs the download itself on a copy of the live profile (same IP + UA + TLS fingerprint the clearance was minted to):
  ...
  - **Opt-in, OFF by default** — the fallback launches the user's own browser on a copy of their profile, so nothing is installed without explicit consent: `moxie config set browser_fallback auto|chrome|firefox` (persistent) or `MOXIE_BROWSER=` (per run; 1/true = auto). No browser is ever shipped or downloaded — both engines launch the user's installed browser (`launcher.New()`, never rod's `NewBrowser()` download path). With the fallback disabled, challenge errors carry the enable command.
  - Live-verified 2026-08-09: real Firefox 153.0.1 headless through the full pipeline (1 MiB payload, 1.65 s); installer wiring live-checked. Known limitation: raw-launch only auto-downloads URLs that start a download on navigation — vikingfile/datanodes free-download buttons need future CDP click automation (F95-1ng2).
