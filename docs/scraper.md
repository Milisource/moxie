...
|---|---|---|
| `GET /sam/checker.php?threads={csv}` | **Bulk version lookup** — up to 100 IDs per request (101+ → `Invalid threads data or >100`) | `{"status":"ok","msg":{"106408":"v6.3"}}`; `"Unknown"` for untracked threads; untracked threads absent from `msg`. A chunk whose threads are ALL untracked answers `404 {"status":"error","msg":"Thread not found"}` — treated as an empty chunk, not a bulk-pass failure |
| `GET /sam/latest_alpha/latest_data.php?cmd=list&cat=games&search={q}&sort=likes&rows=15` | **Title search** (Redis-backed full-text; needs stopword stripping + 30-char cap, see `SanitizeSearchQuery`) | `msg.data[]` with `thread_id`, `title`, `version`, `prefixes`, `tags`, `cover`. `version` may arrive as a bare JSON number (e.g. `1.482`) — parsed leniently (`rawJSONString`), never allowed to invalidate the response |
...
3. Search F95Zone — **primary:** the latest-updates search (`latest_data.php?cmd=list&search={q}`, with `SanitizeSearchQuery` stopword stripping), carrying the session cookie when one is available (lifts the hard anonymous per-hour quota); **fallback:** the XenForo POST search (`/search/search` with `_xfToken`) and then Google site search. When the primary query returns no results, `PublicAPI.SearchTitleFirstHit` walks a **query-variant ladder** (`SearchQueryVariants`, F95-b6q5) — trailing noise tokens dropped ("Monster Black Market w DLC w Uncen" → "Monster Black Market"), trailing digits stripped ("SiNiSistar2" → "SiNiSistar"), camel-case split ("CyanBrain" → "Cyan Brain"), then progressive word drops from both ends (down to 2 words) — so real games with messy directory names still get found. Every variant result is still scored ≥ 0.3 against the local title and engine-checked, so recall improves without new false associations. The XenForo POST requires a fresh `_xfToken` fetched from page HTML (`GET /` + regex on the hidden `_xfToken` input): XenForo 2.3+ rejects the legacy `xf_csrf` cookie value with HTTP 400 ("Security error occurred"), which previously sent every search down the Google fallback. The fetched token is cached for 10 minutes; a 400 on the search POST invalidates the cache and retries once with a fresh token before falling back. HTTP 4xx responses are not retried (client errors), 5xx/429/network errors are. XenForo search result rows carry no game image — only the poster's avatar (`contentRow-figure`), which is deliberately NOT surfaced as a thumbnail; the desktop browser attaches real cover art from the F95Checker catalog (`PublicAPI.SearchCovers`, matched by `ThreadIDFromURL`), so unmatched rows (mods, requests) show the UI placeholder instead of a stranger's profile picture.
...
5. Score each candidate with `ComputeMatchScore()`:
   ...
   - Both sides are camel-case split before tokenization (`camelSplitWords`), so directory names like `FreeEmberDoors` / `CyanBrain` score against their spaced thread titles instead of 0.0 (F95-b6q5)
   - `VersionMatchBonus` (+0.3, capped) when the local version and a candidate's thread version share their leading release segments ("1.3.0" vs "v1.3.1") — breaks title-score ties between a game and its versioned sequel (`SiNiSistar2` local v1.3.0 picks "SiNiSistar 2" v1.3.1 over the original "SiNiSistar" v3.0.1, F95-b6q5)
...
7. **Primary:** fetch full thread data from the F95Checker cache API (version, status, developer, cover, and engine via the `type` enum — the correct numbering, unlike latest_data.php's `prefixes`); **fallback:** scrape the winning thread page directly and save metadata. Engine consistency is checked against the cache `type` field first, prefix IDs second (F95-ytru). **Covers are fetched at full resolution:** the cache API's `image_url` and the thread scrape both point at `attachments.f95zone.to` originals (AVIF/WEBP/JPEG, 560px+), never the 400px `preview.f95zone.to` preview CDN — `PublicAPI.SearchCovers` rewrites preview URLs to the attachments host (same path, no extra request). Thread-page extraction (`extractCoverImage`) handles XenForo's lazy-loaded posts: the `<img src>` is an SVG placeholder, so the real URL is read from `data-src` / the `.lbContainer-zoomer` lightbox div, and gallery thumbnails (`thumb/` subpath) are replaced by the full-size original their anchor links to.

**Status inference (`ResolveStatus`, F95-v9lz):** F95Zone threads carry no "active" tag — a thread tagged Completed/Abandoned/On Hold is unambiguous, but the *absence* of those tags means the game is in active development (the same inference F95Checker makes). Every association/update path funnels the parsed status through `ResolveStatus(parsed, current)`: an explicit parsed status always wins; otherwise an empty/`unknown` stored status is defaulted to `active`; a stored non-unknown status (typically user-set via the TUI/desktop) is never clobbered by the inference. The cache-API metadata refresh additionally re-fetches full thread data for games whose status is still unknown even when their thread has not changed since the previous check — previously an unknown status could only be fixed by the thread changing (or a forced sync).
