...
- [x] Association recall ladder (F95-b6q5) — `SearchTitleFirstHit` walks query variants when the primary search finds nothing (trailing noise/version tokens dropped, camel-case split, progressive word drops); `ComputeMatchScore` is camel-aware and `VersionMatchBonus` breaks sequel ties; `SanitizeSearchQuery` cuts at word boundaries instead of emitting partial tokens
...
- [x] **AVIF covers** — F95Zone's CDN serves AVIF-encoded images under `.png`/`.jpg` cover URLs (Cloudflare conversion); `knownImageFormat`/`imageMimeFromPrefix` sniff the ISOBMFF `ftyp`+`avif`/`avis` brand so AVIF covers are cached and served (`image/avif`). Thumbnails are skipped for AVIF (no pure-Go decoder; cover server falls back to the full image). `decodeCoverImage` returns a typed `errCoverFormatNotThumbnailable` for AVIF instead of the misleading stdlib `image: unknown format`; `writeCoverThumb` reports `thumbSkipAVIF` and `backfillCoverThumbs` aggregates skips into the `cover thumbnails backfilled` summary (`skippedAVIF`/`skippedOther`) — no per-cover WARN storm at startup (F95-x2mg)
...
- [x] **Search & covers overhaul (2026-08-09)**:
  - **XenForo search fixed** — XenForo 2.3+ rejects the legacy `xf_csrf` cookie value as a POST token (HTTP 400 "Security error occurred"), which silently sent every search down the Google fallback (zero results). `SearchF95Zone` now fetches a fresh `_xfToken` from page HTML (`GET /` + hidden-input regex), caches it 10 minutes, refetches once when a 400 comes back, then falls back. HTTP 4xx is no longer retried (client errors only 5xx/429/network get retries); `NewUnsafeClient` also skips search pacing for tests
  - **Real cover art on search cards** — XenForo search result rows only carry the poster's avatar; `parseSearchResults` no longer surfaces it as a thumbnail. The desktop browser attaches actual covers from the F95Checker catalog (`PublicAPI.SearchCovers` keyed by `ThreadIDFromURL`), so mods/requests threads show the placeholder instead of a stranger's profile picture
  - **Full-resolution covers everywhere** — `SearchCovers` rewrites `preview.f95zone.to` (F95Checker's 400px preview CDN) to `attachments.f95zone.to` (same path, 560px+ originals, AVIF). `extractCoverImage` understands XenForo lazy-loaded posts (`data-src` / `.lbContainer-zoomer` lightbox divs, `thumb/` → full-size anchor links) — the thread-scrape fallback and thread preview previously returned **no cover at all** on modern threads
  - **Browser search is explicit-only** — typing never auto-triggers a search (300 ms debounce removed) and remounting the tab no longer re-runs a persisted query; Search button / Enter only. Stale-response guards (`searchSeq`) still make rapid re-searches race-safe
  - **Library cover cells are landscape** — 72×40 (was 40×56 portrait), so banner art isn't cut off; the cover column widened 50→80px; detail-view cover max-width 260→320px with a 16:9 placeholder
...
- [x] Wails Desktop App — Alpha (F95-1132)
  ...
  - [x] Cover art thumbnails in game list — lazy-loaded from a loopback HTTP cover server (320px JPEG thumbs generated at cache time, full image in the detail view); landscape 72×40 cells so banner art isn't cropped, full-res attachments originals (see "Search & covers overhaul")
  ...
  - [x] F95Zone game browser with search, preview panel, add-to-library — explicit-only search (button/Enter, no typing debounce), covers from the F95Checker catalog at full resolution
  ...
  - [x] **Library as a collection (P0, F95-6ail)** — the library default is now a cover grid (list stays as an explicit toggle): plaintext titles under covers, update / not-installed play-state badges, hover Play per card, and state-switching Play in both the grid and the list. Default arrangement is **recently played** (from `play_history`, most-recent first, never-played grouped last); **sort by date added** is a first-class option (moxie owns `created_at`). Primary quick-view tabs **All · Installed · Ready to play · Recently played** sit above the secondary engine/status filters. Play-state is derived from existing summary data (`/virtual/` paths, update-busy phases); recency/date-added read optional per-game timestamps (`createdAt`/`lastPlayed`, mirroring `db.Game.created_at` / `DesktopPlayEntry.played_at`) and degrade gracefully when absent — a follow-up backend change should surface them on `DesktopGameSummary`.
- [x] All background operations keep their state across tab switches — every Wails event subscription now lives in the app shell (App.svelte): the app self-update download (`update:*`), the game install pipeline (`game-install:*`), and the per-game update pipeline (`game-update:*`) all keep their progress and button states alive when visiting other tabs. The detail view derives its Update/Install buttons from that shared state (one source of truth for the backend's global single-run lock shared by updates and installs), and the status bar reports the running pipeline from any tab. The Updates tab's check-only "App Update Checker" was replaced by a pointer to the full flow in Settings (F95-4y1k)
  ...
  - [x] **Browser fallback for challenge-graded hosts (F95-675j)** — `HostResolver.SetBrowserFallback`/`SetDefaultBrowserFallback` hook invoked once per download on Cloudflare challenges (file-hop Cf-Mitigated / 403 challenge body, or resolver-stage challenge/captcha failure); **masked-URL unwrap failures now propagate** the real error (F95Zone answers `{"status":"captcha"}` intermittently — challenge-marked errors trigger the browser fallback instead of the misleading "could not extract file ID"); `browserresolve` gains a **raw-launch Firefox engine** (zero deps: profiles.ini `Default=1` discovery, profile copy keeping cookies.sqlite+WAL, user.js download prefs, `--headless -profile <copy> -no-remote`, `.part`-aware download polling, headless→headful escalation with xvfb-run, process-tree teardown per OS), Edge/Brave profile roots, Playwright-cache chromium discovery, **rod click automation** (masked `host_link` Continue, free-download buttons by text/href/id, reCAPTCHA checkbox in its iframe), **CDP session-cookie injection from any browser store** (Firefox cookies powering a Chromium session), and per-URL engine selection (masked URLs → rod; cookie-holder → Chrome → Firefox; `MOXIE_BROWSER=auto|chrome|firefox` env / `browser_fallback` config key, `off` disables; **nothing is shipped or downloaded — both engines launch the user's own installed browser**). Wired into TUI download, CLI `download`, and the desktop app. Live-verified 2026-08-09: Firefox 153 headless E2E; Chromium (Playwright cache) through masked-Continue and reCAPTCHA-checkbox click flows; **real Meltys Quest install** (game 135: masked unwrap → pixeldrain API → 1.17 GB → extract 2563 files → merge) — plus CLI download-record status fix (records no longer stuck at "extracting")
...
- **Download feature is BETA** — Verified working without a browser (2026-08-09): Pixeldrain, Mediafire, Workupload, Gofile, Google Drive, Catbox, **Buzzheavier** (token flow, live-verified: resolve + file hop), masked-link unwrap + megatools. **Browser fallback (F95-675j)**: challenge-graded hosts download inside the user's real browser (Chrome-family via rod with **click automation** — masked Continue, free-download buttons, reCAPTCHA checkbox — plus CDP cookie injection from any browser store; or raw-launch Firefox for auto-download URLs; per-URL engine selection; `MOXIE_BROWSER=auto|chrome|firefox` or `browser_fallback` config) when the Go path hits a Cloudflare challenge — nothing is shipped or downloaded, both engines use the user's installed browser. F95Zone's masked-URL wall is an **intermittent captcha** (unwrap answers `ok` or `captcha` per attempt); captcha answers propagate as challenge errors and trigger the browser fallback. Remaining wall: raw-launch **Firefox cannot click** (no DOM access) — click-required hosts need the Chrome-family engine (F95-1ng2). uTLS was verified NOT to help (A/B: stdlib 4/4 pass vs uTLS 4/4 challenged — the h1-only ALPN makes an "impossible browser"). The fallback loop tries all available links and shows detailed per-host errors. When all links fail, use `moxie install <id> <path>` with a manually downloaded archive — the desktop app offers the same path via the "Provide file…" button in the Updates view and detail view.
